...
public class Tools : MonoBehaviour
{
    ...
    private void OnGUI()
    {
        if (GUI.Button(new Rect(m_StartX, m_StartY, m_Width, m_Height), "Lua Memory"))
        {
            var count = LuaClient.GetMainState().LuaGC(LuaGCOptions.LUA_GCCOUNT);
            Debug.Log("Lua Memory = " + count);
        }
        else if (GUI.Button(new Rect(m_StartX, m_StartY + m_Space, m_Width, m_Height), "Lua GC"))
        {
            LuaClient.GetMainState().LuaGC(LuaGCOptions.LUA_GCCOLLECT);
            Debug.Log("Call Lua GC");
        }
        else if (GUI.Button(new Rect(m_StartX, m_StartY + m_Space * 2, m_Width, m_Height), "C# GC"))
        {
            System.GC.Collect();
            Debug.Log("Call C# GC");
        }
        else if (GUI.Button(new Rect(m_StartX, m_StartY + m_Space * 3, m_Width, m_Height), "Load Scene"))
        {
